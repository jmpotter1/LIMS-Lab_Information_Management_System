def integral_cryptanalysis(key, plaintext, ciphertext, s_box, p_box, round_key):
    # Initialize the integral table
    integral_table = [[0] * 256 for _ in range(256)]
        ...
        for output_diff in range(256):
            ...
            for plain in range(256):
                if s_box[plain ^ input_diff] ^ s_box[plain] == output_diff:
                    count += 1
            integral_table[input_diff][output_diff] = count
    ...
    for key_diff in range(256):
        ...
        # Check if the ciphertext difference matches any entry in the integral table
        for input_diff in range(256):
            if integral_table[input_diff][cipher_diff] > 0:
...
integral_cryptanalysis(key, plaintext, ciphertext, s_box, p_box, round_key)